...
const appName = "opsgenie_alert_action";
const appNamespace = {
    owner: "nobody",
    app: appName,
    sharing: "app",
};
const pwRealm = "opsgenie_alert_action";
const pwName = "password";
const regionName = "region";  // New variable for the region key
...
require([
    ...
    async function completeSetup() {
        ...
        const regionToSave = $('#region_select').val();  // Get the selected region
        ...
        try {
            ...
            const regionKey = `${pwRealm}:${regionName}:`;
            ...
            function passwordCallback(err, resp) {
                ...
                // Save the region after the password is successfully saved
                stage = 'Saving region selection';
                saveRegion(regionToSave, service, passwords, regionKey);
    ...
    async function saveRegion(region, service, passwords, regionKey) {
        let stage = `Checking for existing region for key = ${regionKey}`;
        try {
            const existingRegion = passwords.item(regionKey);
            await existingRegion;

            if (!existingRegion) {
                // Region doesn't exist, create new one
                stage = `Creating a new region entry for key = ${regionKey}`;
                ...
                // Region exists, update to new value
                stage = `Updating existing region for key = ${regionKey}`;
                await existingRegion.update(
                    {
                        password: region,
                    });
            }
        } catch (e) {
            console.warn(`Error encountered during stage: ${stage}`, e);
            throw e;  // Re-throw the error to be handled in the main function
        }
    }
